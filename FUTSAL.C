...
void booking(ch)
{
	...
	switch(ch)
	{
		...
		{
			FILE *fp1;
			int i,recsize,year1,month1,day1,time1,stime,flag=0;
			char c;
			...
			while(another == 'y')
			{
				...
				if(detail.d.year <=2021 || detail.d.month < 1 || detail.d.month > 12 || detail.d.day < 1 || detail.d.day > 30){
				...
				fflush(stdin);
				if(detail.stime < 8 || detail.stime > 19 || detail.etime < detail.stime || detail.etime > 19 || detail.etime < 8)
				{
...
//				fwrite(&detail,recsize,1,fp1);
				...
				{
					if(detail.d.year == year1 && detail.d.month == month1 && detail.d.day == day1 && time1 >= detail.stime && time1 <=detail.etime)
					{
						flag = 1;
						gotoxy(15,20);
...
void find_booking()
{
	...
	while(fread(&detail,sizeof(detail),1,fp2) >0)
	{
		if(strcmp(uname,detail.name)==0 && unum == detail.num)
		{
			...
			gotoxy(16,16);
			printf("Contact No. :%ld",detail.num);
			gotoxy(16,17);
...
void modify_booking()
{
	...
	FILE *fp6;
	int recsize,year3,month3,day3,i,mflag = 0;
	long int unum2;
	char uname2[50],c;
	c_again2:
	...
	while(fread(&detail,sizeof(detail),1,fp6)>0)
	{
		if(strcmp(uname2,detail.name)==0 && unum2 == detail.num)
		{
			...
			printf("Booking Complete!!");
			gotoxy(20,17);
			printf("Do you want modify another booking(y/n) : ");
			c = getche();
			if(c=='y' || c=='Y')
			{
				goto c_again2;
